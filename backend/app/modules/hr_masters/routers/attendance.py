...
async def create_employee_checkin(data: schemas.EmployeeCheckinCreate, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def list_employee_checkins(db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id), skip: int = 0, limit: int = 100):
...
async def get_employee_checkin(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def create_attendance(data: schemas.AttendanceCreate, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def list_attendance(db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id), skip: int = 0, limit: int = 100):
    stmt = select(models.Attendance).options(selectinload(models.Attendance.employee)).where(models.Attendance.tenant_id == tenant_id).offset(skip).limit(limit)
    result = await db.execute(stmt)
    return result.scalars().all()

@attendance_router.get("/{id}", response_model=schemas.AttendanceResponse)
async def get_attendance(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def submit_attendance(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def cancel_attendance(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
    stmt = select(models.Attendance).where(models.Attendance.id == id, models.Attendance.tenant_id == tenant_id)
    result = await db.execute(stmt)
    obj = result.scalar_one_or_none()
    if not obj:
...
async def create_attendance_request(data: schemas.AttendanceRequestCreate, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def list_attendance_requests(db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id), skip: int = 0, limit: int = 100):
    stmt = select(models.AttendanceRequest).options(selectinload(models.AttendanceRequest.employee)).where(models.AttendanceRequest.tenant_id == tenant_id).offset(skip).limit(limit)
    result = await db.execute(stmt)
    return result.scalars().all()

@attendance_request_router.get("/{id}", response_model=schemas.AttendanceRequestResponse)
async def get_attendance_request(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def submit_attendance_request(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
...
async def approve_attendance_request(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
    stmt = select(models.AttendanceRequest).where(models.AttendanceRequest.id == id, models.AttendanceRequest.tenant_id == tenant_id)
    result = await db.execute(stmt)
    obj = result.scalar_one_or_none()
    if not obj:
...
async def reject_attendance_request(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
    stmt = select(models.AttendanceRequest).where(models.AttendanceRequest.id == id, models.AttendanceRequest.tenant_id == tenant_id)
    result = await db.execute(stmt)
    obj = result.scalar_one_or_none()
    if not obj:
...
async def cancel_attendance_request(id: UUID, db: AsyncSession = Depends(get_db), tenant_id: UUID = Depends(get_tenant_id)):
    stmt = select(models.AttendanceRequest).where(models.AttendanceRequest.id == id, models.AttendanceRequest.tenant_id == tenant_id)
    result = await db.execute(stmt)
    obj = result.scalar_one_or_none()
    if not obj:
